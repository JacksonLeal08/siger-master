...
export const ViaturaModal: React.FC<ViaturaModalProps> = ({
  ...
  return (
      ...
      footer={
        <div className="flex w-full items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {modalSubTab === 'HISTORICO_OS' ? 'Fechar' : 'Cancelar'}
            </button>
            {modalSubTab === 'DADOS' && (
              <button
                type="button"
    ...
    >
      {/* Abas Superiores do Modal da Viatura (quando em edição) */}
      {viaturaToEdit && (
        <div className="flex items-center gap-2 border-b border-slate-200 pb-3 mb-4 font-sans">
          <button
            type="button"
            onClick={() => setModalSubTab('DADOS')}
          ...
          >
            ...
            <span>Dados Cadastrais & Ficha Técnica</span>
          </button>
          
          <button
            type="button"
            onClick={() => setModalSubTab('HISTORICO_OS')}
            className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 ${
              modalSubTab === 'HISTORICO_OS'
                ? 'bg-slate-900 text-white shadow-xs'
                : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
            }`}
          >
            <Wrench className="w-3.5 h-3.5 text-blue-500" />
            <span>Histórico de Manutenções & Ordens de Serviço</span>
          </button>
        </div>
      )}
