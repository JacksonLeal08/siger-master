...
export default function PillarCard3D({ pillar, onOpenDetail }: PillarCard3DProps) {
  ...
  return (
    <div className="relative group/card select-none">
      ...
      >
        <div className="relative bg-white/95 dark:bg-zinc-950/95 border border-slate-200 dark:border-zinc-700/80 rounded-xl px-3.5 py-2 shadow-xl dark:shadow-2xl backdrop-blur-md whitespace-nowrap flex items-center gap-2">
          <Sparkles className="w-3 h-3 text-[#1C4E26] dark:text-[#68D346] animate-pulse shrink-0" />
          <span className="text-[11px] font-mono text-slate-800 dark:text-zinc-200 font-medium">
            {pillar.tooltipText}
          </span>
          {/* Caret / Seta apontando para o centro do card */}
          <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-3 h-3 bg-white dark:bg-zinc-950 border-r border-b border-slate-200 dark:border-zinc-700/80 rotate-45" />
        </div>
      </div>
